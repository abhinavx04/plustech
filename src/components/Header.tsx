    ...
    <header className={`fixed top-0 w-full z-50 transition-all duration-500 ${
      ...
      <div className="container mx-auto px-8">
        <div className="flex justify-between items-center py-5">
          ...
          <div className="flex items-center">
            <Link to="/" className="flex items-center space-x-3 group">
              <img 
                ...
                alt="PlusTech Logo" 
                className="h-13 w-40 object-cover transition-all duration-300 group-hover:scale-105"
              />
            </Link>
          </div>
              ...
              item.dropdown ? (
                <div key={index} className="relative">
                  <button
                    ...
                    className="flex items-center px-7 py-3 text-gray-700 hover:text-blue-600 transition-all duration-300 font-medium rounded-xl hover:bg-blue-50 relative group"
                  >
                    {item.name}
                    ...
                    {item.dropdown.map((sub, subIdx) => (
                      !sub.sub ? (
                        <button
                          ...
                          className="block w-full text-left px-7 py-3 text-gray-700 hover:text-blue-600 hover:bg-blue-50 transition-all duration-200 text-sm font-medium group relative rounded-lg mx-2"
                        >
                        ...
                        <div key={subIdx} className="px-7 py-3 border-b border-gray-200/50 last:border-b-0">
                          ...
                          {sub.sub.map((subsub, subsubIdx) => (
                            <button
                              ...
                              className="block w-full text-left pl-5 py-2 text-gray-600 hover:text-blue-600 hover:bg-blue-50 transition-all duration-200 text-sm rounded-lg group relative"
                            >
                              <span className="flex items-center">
          ...
          <div className="hidden lg:flex items-center space-x-7">
            ...
            <div className="flex items-center space-x-5">
              <div className="flex items-center space-x-3 text-sm text-gray-600 hover:text-blue-600 transition-colors cursor-pointer group">
        ...
        <div className={`lg:hidden transition-all duration-500 overflow-hidden ${
          ...
          <nav className="border-t border-gray-200 pt-5">
            <div className="flex flex-col space-y-1">
              {navItems.map((item, index) => (
                item.dropdown ? (
                  <div key={index} className="relative">
                    <button
                      onClick={() => handleDropdownToggle(item.name)}
                      className="flex items-center justify-between w-full px-5 py-4 text-gray-700 font-medium cursor-pointer hover:text-blue-600 hover:bg-blue-50 rounded-xl transition-all duration-200"
                    >
                      {item.name}
                      <ChevronDown className={`h-4 w-4 transition-transform duration-200 ${activeDropdown === item.name ? 'rotate-180' : ''}`} />
                    </button>
                    <div className={`pl-5 overflow-hidden transition-all duration-300 ${
                      ...
                      {item.dropdown.map((sub, subIdx) => (
                        !sub.sub ? (
                          <button
                            key={subIdx}
                            onClick={() => handleDropdownItemClick(sub.hash, sub.label)}
                            className="block w-full text-left py-3 px-5 text-gray-600 hover:text-blue-600 hover:bg-blue-50 transition-all duration-200 text-sm rounded-xl group relative"
                          >
                            <span className="flex items-center">
                              <div className="w-1 h-1 bg-gray-400 rounded-full mr-3 group-hover:bg-blue-500 transition-colors"></div>
                              {sub.label}
                          ...
                          <div key={subIdx} className="mb-2">
                            <div className="font-semibold text-gray-800 mb-1 px-5 py-2 flex items-center">
                              <span>{sub.label}</span>
                              <div className="ml-2 w-2 h-2 bg-blue-500 rounded-full"></div>
                            </div>
                            {sub.sub.map((subsub, subsubIdx) => (
                              <button
                                key={subsubIdx}
                                onClick={() => handleDropdownItemClick(subsub.hash, subsub.label)}
                                className="block w-full text-left pl-10 py-2 text-gray-600 hover:text-blue-600 hover:bg-blue-50 transition-all duration-200 text-sm rounded-xl group relative"
                              >
                                <span className="flex items-center">
                                  <div className="w-1 h-1 bg-gray-400 rounded-full mr-3 group-hover:bg-blue-500 transition-colors"></div>
                                  {subsub.label}
                  ...
                  <Link 
                    key={index}
                    to={item.to}
                    className="text-gray-700 hover:text-blue-600 hover:bg-blue-50 px-5 py-4 rounded-xl transition-all duration-200 font-medium"
              ...
              <div className="pt-6 border-t border-gray-200 space-y-4">
                <div className="flex items-center space-x-3 text-sm text-gray-600 px-5">
